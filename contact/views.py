...
class IdentityAPIView(APIView):
    ...
    def post(self, request):
        ...
        if (validated_email == '' and validated_phone == ''):
            return Response(status=HTTP_400_BAD_REQUEST, data={'Atleast one field is required among email and phoneNumber'})

        if validated_phone and validated_email:
            primary_contact = Contact.objects.get_or_create_phone_email(validated_phone, validated_email)
        ...
        secondary_contacts = Contact.objects.filter(primary_contact__id=primary_contact.id)
        aggregated_contacts = aggregate_linked_contacts(primary_contact, secondary_contacts)

        data = {
            'contact': aggregated_contacts
        }
